#!/usr/bin/env python3
"""Config-driven Firestore to PostgreSQL migration script."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Migrate Firestore collections/documents into PostgreSQL tables."
    )
    parser.add_argument(
        "--config",
        required=True,
...
def initialize_firestore(service_account_path: str, project_id: str) -> firestore.Client:
    ...
    app = firebase_admin.initialize_app(credential, options=options or None)
...
def flush_batch(
    ...
    try:
        ...
        conn.rollback()
        # Retry row-by-row to identify the exact failing document.
        ...
        # If all single-row retries pass, surface original batch error.
        raise
...
def main() -> int:
    ...
    batch_size = int(args.batch_size or config.get("batch_size", 500))
    if batch_size <= 0:
        raise ValueError("Batch size must be > 0.")

    firestore_client = initialize_firestore(args.service_account, args.project_id)
